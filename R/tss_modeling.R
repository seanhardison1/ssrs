...
# comparison with GAM
mod1 <- gam(daily_TSS ~ s(vals) + s(Longitude, Latitude, k= 50),
...
gam_mod <- gam(daily_TSS ~ s(vals) + s(Longitude, Latitude, k= 50),
               family = Gamma("log"), data = tss_train, method = "ML")
...
grf_mod <- sdmTMB(daily_TSS ~ s(vals),
             family = Gamma("log"),
             data = tss_train,
             spde = cb_spde, 
             spatial_only = TRUE)
